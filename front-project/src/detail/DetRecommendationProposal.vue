...
const fetchPostDetails = async () => {
 try {
  await axios.put(`/proposal/${route.params.id}/view`);
   const response = await axios.get(`/proposal/${route.params.id}`);
   post.value = response.data;
   console.log('Post Data:', post.value); // 응답 데이터 확인

   const commentsResponse = await axios.get(`/proposal/${route.params.id}/comments`);
...
const editPost = () => {
 router.push({ name: 'ModDeRecommendationProposal', params: { id: route.params.id } });
};
...
const deletePost = async () => {
 ...
 try {
   await axios.delete(`/proposal/${post.value.id}`);
   router.push({ name: 'RecommendationProposal' }); // 게시글 목록 페이지로 이동
 } catch (error) {
