<template>
  ...
  <div class="project-container">
    <h1 class="project-title">발표 자료</h1>
    <div class="nav-buttons">
      ...
      <router-link class="nav-button" :to="{ name: 'ActivityPlan' }"
        >수행 계획서</router-link
      >
      <router-link class="nav-button active" :to="{ name: 'Presentation' }"
        >발표 자료</router-link
      >
      ...
      <div class="search-container">
        ...
        <button @click="searchPosts" class="search-btn">
          ...
          >
            ...
            <5l2h d="M14.4121122,14.4121122 L20,20" />
    ...
    <table class="posts-table">
      ...
      <tbody>
        <tr v-for="post in filteredPosts" :key="post.id" class="post-item">
          ...
          <!-- 제목 클릭 시 상세 페이지로 이동 -->
          <td>
            <router-link :to="{ name: 'DetPresentation', params: { id: post.id } }">{{
              post.title
            }}</router-link>
          </td>
          <td>{{ post.userId }}</td>
...
const handleClick = async () => {
  if (isLoggedIn.value) {
    if (isAdmin.value) {
      router.push({ name: "WrDePresentation" });
    } else {
...
<style scoped src="@/assets/style/fourthmenu/Present.css"></style>