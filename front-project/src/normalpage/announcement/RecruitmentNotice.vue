<template>
  ...
  <router-link class="ggg" :to="{ name: 'ApplyForAdmission' }"
      >《신청 바로가기!》</router-link>
 ...
 <table class="posts-table">
    ...
    <tbody>
      <tr v-for="post in filteredPosts" :key="post.id" class="post-item">
        ...
        <td>
          <router-link
            :to="{ name: 'DetRecruitmentNotice', params: { id: post.id } }"
...
const handleLogin = async () => {
  ...
  if (response.data.message) {
    ...
    if (userStore.isAdmin) {
      router.push({ name: "WrPrRecruitmentNotice" }); // 관리자가 접근하는 페이지
    } else {
      router.push({ name: "WrPrRecruitmentNotice" }); // 일반 회원이 접근하는 페이지
    }
  } else {
...
<style scoped src="@/assets/style/fifthmenu/ReNotice.css"></style>
