...
<h1 class="announcement-title">문의게시판</h1>
<div class="nav-buttons">
  ...
  <router-link class="nav-button" :to="{ name: 'Employment' }"
    >채용공고</router-link
  >
  <router-link class="nav-button active" :to="{ name: 'QnABoard' }"
    >문의게시판</router-link
  >
</div>
...
const commentsResponse = await axios.get(
  `/qnaboard/${route.params.id}/comments`
);
...
const editPost = () => {
router.push({ name: "ModDeQnABoard", params: { id: route.params.id } });
};
...
try {
await axios.delete(`/qnaboard/${post.value.id}`);
router.push({ name: "QnABoard" }); // 게시글 목록 페이지로 이동
} catch (error) {
...
try {
await axios.post(
  `/qnaboard/${post.value.id}/comments`,
  {
...
const deleteComment = async (commentId) => {
  try {
    await axios.delete(`/qnaboard/${post.value.id}/comments/${commentId}`);
    fetchPostDetails(); // 댓글 리스트 갱신
  } catch (error) {
