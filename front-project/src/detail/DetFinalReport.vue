...
const fetchPostDetails = async () => {
 try {
  await axios.put(`/finalreport/${route.params.id}/view`);
   ...
   const commentsResponse = await axios.get(`/finalreport/${route.params.id}/comments`);
...
const editPost = () => {
 router.push({ name: 'ModDeFinalReport', params: { id: route.params.id } });
};
...
const deletePost = async () => {
 ...
 try {
   await axios.delete(`/finalreport/${post.value.id}`);
   router.push({ name: 'FinalReport' }); // 게시글 목록 페이지로 이동
 } catch (error) {
