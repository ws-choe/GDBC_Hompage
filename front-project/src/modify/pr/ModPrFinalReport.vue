    ...
    <!-- 고정 페이지 최상단 영역 -->
         ...
         <div class="project-container">
           <div class="nav-buttons">
             ...
             <router-link class="nav-button" :to="{ name: 'DeFinalReport' }">최종 보고서</router-link>
             <router-link class="nav-button" :to="{ name: 'DeCode' }">코드</router-link>
         </div>
         ...
         <div class="search-container">
             ...
             <!-- 글 쓰기 버튼 (관리자만) -->
             <button @click.prevent="handleClick" class="write"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-pencil" viewBox="0 0 16 16">
         ...
         <table class="posts-table">
             ...
             <tbody>
               <tr v-for="post in filteredPosts" :key="post.id" class="post-item">
                 <td>{{ post.id }}</td>
                 <!-- 제목 클릭 시 상세 페이지로 이동 -->
                 <td><router-link :to="{ name: 'DeModCourseReviews', params: { id: post.id } }">{{ post.title }}</router-link></td>
                 <td>{{ post.userId }}</td>
