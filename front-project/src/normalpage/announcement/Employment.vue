...
<table class="posts-table">
  ...
  <tbody>
    <tr v-for="post in filteredPosts" :key="post.id" class="post-item">
      ...
      <td>
        <router-link
          :to="{ name: 'DetEmployment', params: { id: post.id } }"
...
const fetchPosts = async (kw, category) => {
  try {
    const response = await axios.get("/employmentboard", {
...
if (isLoggedIn.value) {
  if(isAdmin.value){
  router.push({ name: "WrPrEmployment" });
  }
  else {
...
if (response.data.message) {
  ...
  if (userStore.isAdmin) {
    router.push({ name: "WrPrEmployment" }); // 관리자가 접근하는 페이지
  } else {
    router.push({ name: "WrPrEmployment" }); // 일반 회원이 접근하는 페이지
  }
} else {
...
<style scoped src="@/assets/style/fifthmenu/Emp.css"></style>