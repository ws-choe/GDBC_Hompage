  ...
  <div class="comments">
    <div v-for="comment in comments" :key="comment.id" class="comment-item">
      <div class="comment-info">
        ...
        <button class="btn-reply">댓글</button>
      </div>
      <p class="comment-content">{{ comment.content }}</p>
        ...
        <div class="button-group">
          <router-link :to="{ name: 'Code' }" ><button type="submit" class="list-button">목록</button></router-link>
      </div>
      </div>
      </div>
</template>
...
try {
  await axios.put(`/employmentboard/${route.params.id}/view`);
const response = await axios.get(`/employmentboard/${route.params.id}`);
post.value = response.data;
console.log("Post Data:", post.value); // 응답 데이터 확인

const commentsResponse = await axios.get(
  `/employmentboard/${route.params.id}/comments`
);
...
const editPost = () => {
router.push({ name: "ModPrEmployment", params: { id: route.params.id } });
};
...
try {
await axios.delete(`/employmentboard/${post.value.id}`);
router.push({ name: "Employment" }); // 게시글 목록 페이지로 이동
} catch (error) {
...
try {
await axios.post(`/employmentboard/${post.value.id}/comments`, {
...
const deleteComment = async (commentId) => {
  try {
    await axios.delete(`/employmentboard/${post.value.id}/comments/${commentId}`);
    fetchPostDetails(); // 댓글 리스트 갱신
  } catch (error) {
